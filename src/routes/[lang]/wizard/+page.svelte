...
<BasicLayout>
	<div slot="content" class="mx-auto mt-8 w-full max-w-2xl">
			...
			<div class="mb-6 animate-fade1">
				...
				<div class="space-y-2">
					{#each addedProfiles as profile, index}
						<div
							class="flex items-center gap-3 rounded border-2 border-neutral-200 bg-neutral-50 p-3 dark:border-neutral-700 dark:bg-neutral-800"
						>
							<img
							...
							<button on:click={() => removeProfile(index)} class="p-1 text-accent" title="Remove">
		...
		<div class="mb-10 animate-fade1">
			...
			{#if errorMessage}
				<p class="mt-2 text-sm text-accent">{errorMessage}</p>
			{/if}
		</div>
		...
		{#if addedProfiles.length > 0}
			{#if generatedUrl == ''}
				<div class="mb-6 animate-fade1">
					<button
						...
						class="hover:bg-accent/90 inline-flex w-full items-center justify-center rounded bg-accent px-8 py-4 text-[1.3rem] text-white transition-colors"
					>
						Create link
						<svg
							class="ml-2 h-5 w-5"
			...
			{:else}
				<div class="mb-6 border-t-4 border-t-accent"></div>
			{/if}
		{/if}
			...
			<div class="mb-6 animate-fade1">
				...
				<div class="flex flex-col gap-2 sm:flex-row">
					<input
						...
						value={generatedUrl}
						readonly
						class="flex-1 select-all rounded border-2 border-neutral-300 bg-neutral-50 px-4 py-3 text-[1rem] text-neutral-700 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-300"
					/>
					<button
